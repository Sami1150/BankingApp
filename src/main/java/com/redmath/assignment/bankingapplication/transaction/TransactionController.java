...
//http://localhost:8080/api/v1/transaction/withdrawfunds?accountId=3&amount=150.0&description=Debit%20Transaction
    @PostMapping("/withdrawfunds")
    public ResponseEntity<Transaction> withdrawFunds(@RequestParam long accountId,
                                                     @RequestParam double amount,
                                                     @RequestParam String description) {
        Transaction newTransaction = transactionService.withdrawFunds(accountId, amount, description);
        return ResponseEntity.ok(newTransaction);
    }
